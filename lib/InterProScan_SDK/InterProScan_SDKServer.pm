package InterProScan_SDK::InterProScan_SDKServer;
...
our %async_run_methods = (
        'annotate_genome_with_interpro_pipeline_async' => 'InterProScan_SDK.annotate_genome_with_interpro_pipeline',
);

our %async_check_methods = (
        'annotate_genome_with_interpro_pipeline_check' => 'InterProScan_SDK.annotate_genome_with_interpro_pipeline',
);
...
sub call_method {
    ...
    my $ctx = InterProScan_SDK::InterProScan_SDKServerContext->new($self->{loggers}->{userlog},
...
{
    # Service InterProScan_SDK requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for InterProScan_SDK but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = InterProScan_SDK::InterProScan_SDKServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service InterProScan_SDK.");
    }
...
package InterProScan_SDK::InterProScan_SDKServerContext;

use strict;

=head1 NAME

InterProScan_SDK::InterProScan_SDKServerContext
...
package InterProScan_SDK::InterProScan_SDKServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = InterProScan_SDK::InterProScan_SDKServer->new(
